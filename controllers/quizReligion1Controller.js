...
import quizR1Model from "../models/quizReligionA1.js";

const guardarDatosB= async (req, res) => {
    try {
      const { correo } = req.body;
  
      // Buscar el último intento del usuario por correo y ordenarlo en orden descendente
      const ultimoIntento = await quizR1Model.findOne({ correo })
        .sort({ intento: -1 })
        .exec();
      ...
      const newData = new quizR1Model(req.body);
    ...
    const obtenerOpcionesPorCorreoB = async (req, res) => {
      try {
        const { correo } = req.query;
       
        // Buscar los datos en la base de datos por correo
        const datos = await quizR1Model.findOne({ correo });
     console.log(datos)
        if (!datos) {
    ...
    export { guardarDatosB ,obtenerOpcionesPorCorreoB};