import quizG1Model from "../models/quizGobernateA1.js";

const guardarDatosG = async (req, res) => {
    try {
      const { correo } = req.body;
  
      // Buscar el último intento del usuario por correo y ordenarlo en orden descendente
      const ultimoIntento = await quizG1Model.findOne({ correo })
        .sort({ intento: -1 })
        .exec();
      ...
      const newData = new quizG1Model(req.body);
    ...
    const obtenerOpcionesPorCorreoG = async (req, res) => {
      try {
        const { correo } = req.query;
       
        // Buscar los datos en la base de datos por correo
        const datos = await quizG1Model.findOne({ correo });
     console.log(datos)
        if (!datos) {
    ...
    export { guardarDatosG,obtenerOpcionesPorCorreoG};