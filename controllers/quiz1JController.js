...
import quizJ1Model from '../models/quizJeraquiaA1.js';

// Endpoint para guardar los datos en MongoDB
const guardarDatosC = async (req, res) => {
  try {
    const { correo } = req.body;

    // Buscar el último intento del usuario por correo y ordenarlo en orden descendente
    const ultimoIntento = await quizJ1Model.findOne({ correo })
      .sort({ intento: -1 })
      .exec();
    ...
    const newData = new quizJ1Model(req.body);
  ...
  const obtenerOpcionesPorCorreoC = async (req, res) => {
    try {
      const { correo } = req.query;
     
      // Buscar los datos en la base de datos por correo
      const datos = await quizJ1Model.findOne({ correo });
   console.log(datos)
      if (!datos) {
  ...
  export { guardarDatosC ,obtenerOpcionesPorCorreoC};
