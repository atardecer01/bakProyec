import quizA1Model from "../models/quizArted.js";

const guardarDatosA = async (req, res) => {
    try {
      const { correo } = req.body;
  
      // Buscar el último intento del usuario por correo y ordenarlo en orden descendente
      const ultimoIntento = await quizA1Model.findOne({ correo })
        .sort({ intento: -1 })
        .exec();
      ...
      const newData = new quizA1Model(req.body);
    ...
    const obtenerOpcionesPorCorreoA = async (req, res) => {
      try {
        const { correo } = req.query;
       
        // Buscar los datos en la base de datos por correo
        const datos = await quizA1Model.findOne({ correo });
     console.log(datos)
        if (!datos) {
    ...
    export { guardarDatosA ,obtenerOpcionesPorCorreoA};