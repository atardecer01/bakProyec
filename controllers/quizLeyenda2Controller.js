...
import quizL2Model from "../models/quizLeyendaA2.js";

const guardarDatosF = async (req, res) => {
    try {
      const { correo } = req.body;
      console.log('hola')
  
      // Buscar el último intento del usuario por correo y ordenarlo en orden descendente
      const ultimoIntento = await quizL2Model.findOne({ correo })
        .sort({ intento: -1 })
        .exec();
      ...
      const newData = new quizL2Model(req.body);
    ...
    const obtenerOpcionesPorCorreoF = async (req, res) => {
      try {
        const { correo } = req.query;
       
        // Buscar los datos en la base de datos por correo
        const datos = await quizL2Model.findOne({ correo });
     console.log(datos)
        if (!datos) {
    ...
    export { guardarDatosF ,obtenerOpcionesPorCorreoF};