...
import quizT1Model from "../models/quizTextilA1.js";

const guardarDatosH= async (req, res) => {
    try {
      const { correo } = req.body;
  
      // Buscar el último intento del usuario por correo y ordenarlo en orden descendente
      const ultimoIntento = await quizT1Model.findOne({ correo })
        .sort({ intento: -1 })
        .exec();
      ...
      const newData = new quizT1Model(req.body);
    ...
    const obtenerOpcionesPorCorreoH = async (req, res) => {
      try {
        const { correo } = req.query;
       
        // Buscar los datos en la base de datos por correo
        const datos = await quizT1Model.findOne({ correo });
     console.log(datos)
        if (!datos) {
    ...
    export { guardarDatosH ,obtenerOpcionesPorCorreoH};